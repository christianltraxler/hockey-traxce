...
def addTeamsInfo(db):
    ...
    teamsDict = requests.get('https://statsapi.web.nhl.com/api/v1/teams/').json()['teams']
    ...
    for teamInfo in teamsDict:
        ...
        team = {
            ...
            'logo' : {
                'link': 'https://assets.nhle.com/logos/nhl/svg/' + teamInfo['abbreviation'] + '_light.svg'
            },
...
def getRosters(teamId, startYear, endYear):
    ...
    while (year < endYear):
        ...
        teamRoster = requests.get('https://statsapi.web.nhl.com/api/v1/teams/' + str(teamId) + '/roster?season=' + str(year) + str(year + 1)).json()
...
def getTeamStats(teamId, startYear, endYear): 
    ...
    while (year < endYear):
        ...
        teamStats = requests.get('https://statsapi.web.nhl.com/api/v1/teams/' + str(teamId) + '?expand=team.stats&season=' + str(year) + str(year + 1)).json()
